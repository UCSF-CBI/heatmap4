...
#### If the column or row annotation matrixes contain too many columns to display or there is a need to select only a few, the attribute `col_anno_var` (for column annotation limitations) or `row_anno_var` (for row annotation limitations). The column names for the column annotaion data.frame is: 
...
#### To change the color of both a categorical and continuous variable: In this example, C4 is categorical and C1, C2, R1 are continuous. Only C1 and C4 have been altered.

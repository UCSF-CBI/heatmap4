                             ...
                             file_name = NULL, h_title = NULL,input_legend = c(TRUE, FALSE), legend_title = NULL, heatmap_color=c("red", "blue", "grey"), zlm=c(-0.5, 0.5), ...)
{
  ...
  if (input_legend) {
      ...
      if (T) {
          ...
          heatmapColorBar.my(cols=cols,limit=zlm,k=5)
          ...
          if (input_legend & col_anno) {
            ...
            for (vId in 1:length(col_var)) {
                ...
                if (is.numeric(col_info[ ,col_var[vId]]) & length(unique(col_info[ ,col_var[vId]])) > 5) {
                    ...
                    heatmapColorBar.my(cols=color_vec,limit=lim)
...
```{r}
#basic heatmap; row dend, col dend, row anno, col anno  
...
```{r}
#col anno, col dend only 
generate_heatmap(test, col_lab = TRUE, row_lab = TRUE, col_lab_vtr = NULL, row_lab_vtr = NULL, col_anno = TRUE, row_anno = FALSE, col_info = column_df, row_info = row_df, col_anno_var = c("C1", "C2"), row_anno_var = c("V1"), col_dend =TRUE, row_dend = FALSE)
```

```{r}
#row anno, row dend only
generate_heatmap(test, col_lab = TRUE, row_lab = TRUE, col_lab_vtr = NULL, row_lab_vtr = NULL, col_anno = FALSE, row_anno = TRUE, col_info = column_df, row_info = row_df, col_anno_var = c("C1", "C2"), row_anno_var = c("V1"), col_dend = FALSE, row_dend = TRUE)
```

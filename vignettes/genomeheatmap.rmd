---
title: "genomeheatmap.rmd"
author: "Anishka Dhar"
date: "5/3/2022"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{genome heatmap vignette}
...
A heat map is a false color image (image(t(x))) with a dendrogram added to the left side and to the top. Typically, reordering of the rows and columns according to some set of values (row or column means) within the restrictions imposed by the dendrogram is carried out.
...
We will use the data set genomDat, phen, and anno to map and generate column and row annotations. 
```{r, include = FALSE}
#Load rda file from github package to access data
setwd("~/Downloads/heatmap4-master-2")
load(file = 'data.RData')
genset <- as.matrix(genomDat[1:20,1:20])

# Column annotation df

col<- as.matrix(phen$age..yrs.)
col_rows <- ncol(col)
col_columns <- 2
column_df <- matrix(data = col)
column_df<- as.data.frame(column_df)
colnames(column_df) <- c('age')
col_info <- column_df
...
row_columns <- 1
row_df <- matrix(data = row)
row_df <- as.data.frame(row_df)
colnames(row_df)<- c("chrom")
row_info <- row_df
```

```{r}
#Sample portion of genset (heatmap examples will use [1:20,1:20])
print(genset[1:9,1:6])
...
```{r}
#Default heatmap 
generate_heatmap(genset, input_legend = FALSE)
```

```{r}
#Heatmap with column annotations 
generate_heatmap(genset, col_info = column_df, col_anno = TRUE, input_legend = FALSE)
```

```{r}
#Heatmap with column annotations, column label    
generate_heatmap(genset, col_lab = TRUE, row_lab = FALSE, col_info = column_df, 
col_anno = TRUE, input_legend = FALSE )
```


```{r}
#Heatmap with column annotations, column label, column dendrogram   
generate_heatmap(genset, col_lab = TRUE, row_lab = FALSE, col_info = column_df, 
col_anno = TRUE, col_dend = TRUE, input_legend = FALSE)
```

```{r}
#Heatmap with all annotations
generate_heatmap(genset, col_lab = TRUE, row_lab = TRUE, col_anno = TRUE, 
row_anno = TRUE, col_info = column_df, row_info = row_df, col_dend = TRUE, 
row_dend = TRUE, input_legend = FALSE)
```
...
generate_heatmap(genset, col_lab = TRUE, row_lab = TRUE, col_anno = TRUE, 
row_anno = TRUE, col_info = column_df, row_info = row_df, col_dend = TRUE, 
row_dend = TRUE, input_legend = FALSE, h_title = "Genome Heatmap")
```



```{r}
#Change annotation colors and font size  
main_plot = list("cexCol" = 2, "cexRow" = 2, "cexRowSide" = 1, "cexColSide" = 1)
cate_color = list(age = list(color = c("pink", "white")), chrom = list(color = c("purple",
"white")))

generate_heatmap(genset, col_lab = TRUE, row_lab = TRUE, col_anno = TRUE, row_anno = TRUE,
col_info = column_df, row_info = row_df, col_dend = TRUE, row_dend = TRUE, input_legend = 
FALSE, col_anno_var = c("age"), row_anno_var = c("chrom"), h_title = "Genome Heatmap", 
plot_info = main_plot, col_var_info = cate_color, row_var_info = cate_color)
```

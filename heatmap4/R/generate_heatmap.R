                             ...
                             input_legend = c(TRUE, FALSE), legend_title = NULL, heatmap_color=c("red", "blue", "grey"), zlm=c(-0.5, 0.5), ...)
{
  ...
  for (v in 1:length(row_var)) {
    if (is.numeric(row_info[ ,row_var[v]]) & length(unique(row_info[ ,row_var[v]])) > 5){
      if (is.null(row_var_info)) {		        color_vec <- color_vec_cont_default
      ...
      if (is.numeric(row_info[ ,row_var[v]]) & length(unique(row_info[ ,row_var[v]])) > 5) {
        ...
        if ("limit" %in% names(row_var_info[[row_var[v]]])){
          varib = varib - min(varib, na.rm=T) + 1		            lim <-  row_var_info[[row_var[v]]]$limit
          varib = varib / min(varib, na.rm=T)		            varib=round(varib); varib[varib<lim[1]]=lim[1]; varib[varib>lim[2]]=lim[2]; varib=varib+lim[2]+1
        } else {
          if (F) {
